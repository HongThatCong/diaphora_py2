...
# pylint: disable=C0103,C0301,missing-docstring,bare-except,wildcard-import,unused-wildcard-import
...
class CIDAChooser(CDiaphoraChooser):
    ...
    def show(self, force=False):
        ...
        if self.show_commands and (self.cmd_diff_asm is None or force):
            ...
            self.AddCommand(None)
            # Add by HTC
            self.cmd_copy_address = self.AddCommand("Copy Address")
            self.cmd_copy_name = self.AddCommand("Copy Name")
            self.cmd_copy_address_2 = self.AddCommand("Copy Address 2")
            self.cmd_copy_name_2 = self.AddCommand("Copy Name 2")
            self.AddCommand(None)
    ...
    def OnCommand(self, n, cmd_id):
        ...
        # Add by HTC
        elif cmd_id == self.cmd_copy_address:
            QtWidgets.QApplication.clipboard().setText(self.items[n][1])
        elif cmd_id == self.cmd_copy_name:
            QtWidgets.QApplication.clipboard().setText(self.items[n][2])
        elif cmd_id == self.cmd_copy_address_2:
            QtWidgets.QApplication.clipboard().setText(self.items[n][3])
        elif cmd_id == self.cmd_copy_name_2:
            QtWidgets.QApplication.clipboard().setText(self.items[n][4])
...
class CIDABinDiff(diaphora.CBinDiff):
    ...
    def export(self):
        ...
        try:
            ...
            traceback.print_exc()  # Add by HTC
        finally:
            hide_wait_box()
    ...
    def import_instruction(self, ins_data1, ins_data2):
        ...
        if tmp_ea is not None and type_set:
            if mtype is not None and idc.get_type(tmp_ea) != mtype:
                ...
                SetType(tmp_ea, str(mtype))  # HTC - Force to string type
    ...
    def register_menu_action(self, action_name, action_desc, handler, hotkey=None):
        ...
        idaapi.attach_action_to_menu(
            'View/Diaphora/%s' % action_desc,   # HTC - Change submenu
            action_name,
            idaapi.SETMENU_APP)
...
or selecting View -> Diaphora -> Diaphora - Show results""")
    ...
    def load_results(self, filename):
        ...
        try:
            ...
            main_db = str(row["main_db"])
            diff_db = str(row["diff_db"])
    ...
    def re_diff(self):
        ...
        _diff_or_export(use_ui=True, file_in=self.last_diff_db, project_script=self.project_script)
...
def _diff_or_export(use_ui, **options):
    ...
    if use_ui:
        ...
        # HTC - Free form, avoid IDA warning
        ret = x.Execute()
        if ret:
            opts = x.get_options()
...
class CHtmlDiff(object):
    ...
    def make_file(self, lhs, rhs, fmt, lex):
        ...
        for left, right, changed in difflib._mdiff(lhs, rhs):
            ...
            # Ngon Nguyen: force to Py2 string
            ltxt = unicode(ltxt, "utf-8")
            rtxt = unicode(rtxt, "utf-8")
